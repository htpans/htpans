...
# DIRECTIONS: Only input needed is lines 19 to 25. 

setwd("")
Data_Input_Sheet<-Sample_Survival
Experiment<-"Sample"
Reference_Group<-"Prac1"
x_axisLabel<-"Hours"
x_axisScale<-2
time_interval<-1
...
ExperimentName$Class = factor(ExperimentName$Class)
ExperimentName$Class=relevel(ExperimentName$Class,ref = Reference_Group)




newdata <- ExperimentName
...
SurvivalPlot<-ggsurvplot(sfit,palette = "grey",break.time.by=x_axisScale, conf.int=FALSE, pval=FALSE, risk.table=TRUE,legend.title="", main="Kaplan-Meier Curve",font.legend=10,legend=c("bottom"))
...
CumulativeHazard <- ggsurvplot(sfit, pval=FALSE,break.time.by=x_axisScale,palette = "grey", fun="cumhaz", censor=FALSE,risk.table=FALSE,risk.table.title="Living Neurons",font.x=15,font.y=15,legend.title="",xlab="Day", font.legend=10,legend=c("top"))
