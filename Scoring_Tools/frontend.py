...
class YOLO(object):
    def __init__(self, architecture,
        ...
        #print(self.feature_extractor.feature_extractor.build((self.input_size, self.input_size, 3)))        
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        if self.debug:
            ...
            #self.ref_acc=tf.divide(total_recall,seen)
    ...
    def train(self, train_imgs,     # the list of images to train the model
        ...
        checkpoint = ModelCheckpoint(saved_weights_name, 
                                     ...
                                     mode='min', 
                                     period=1)
        tb_counter  = len([log for log in os.listdir(os.path.expanduser(log_dir)) if 'yolo' in log]) + 1
        tensorboard = TensorBoard(log_dir=os.path.expanduser(log_dir) + 'yolo' + '_' + str(tb_counter), 
                                  histogram_freq=0, 
...
class ref_acc(Callback):
    def __init__(self,tb_counter,log_dir,tf,ref_acc):
        self.log_directory = os.path.expanduser(log_dir) + 'ref_acc' + '_' + str(tb_counter)
        self.tf = tf
        self.ref_acc = ref_acc        
        self.writer = self.tf.summary.FileWriter(self.log_directory)
        self.tag = 'Ref Acc'
        
    def on_train_begin(self, logs={}):
        summary = self.tf.Summary(value=[self.tf.Summary.Value(tag=self.tag,simple_value=0)])
        step=0
    ...
    def on_batch_end(self, batch, logs={}):
        summary = self.tf.Summary(value=[self.tf.Summary.Value(tag=self.tag,simple_value=self.ref_acc)])
        step=batch
        self.writer.add_summary(summary,step)
        self.writer.flush()        
        return
