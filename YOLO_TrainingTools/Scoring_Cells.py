...
def find_cells(file,yolo,qa_path,config,cell_path,image_name,live_dead,data_writer):
    ...
    if image16.shape[0] > image16.shape[2]:
        image16=np.rollaxis(image16,2,0)
...
def find_cells2(file,yolo,qa_path,config,cell_path,image_name,live_dead_first,models,data_writer):
    #yolo is the model, image is the first image int he stack, write_path is where the image with boxes will be written,config is the model info,cell_path is where the cells will be dumped for classification
    image = cv2.imread(file)
    unclassified_cells = scanImage(image,yolo,config)
    if len(unclassified_cells) == 0:
    ...
    classified_cells = remove_dead_cells(cell_images,live_dead_first,unclassified_cells)
    if len(classified_cells) == 0:
        print("no cells")
        return
    out_image = draw_boxes(np.copy(image), unclassified_cells, config['model']['labels'])
    ...
    all_results=[]
    for model in models:
        status = get_cell_status(all_images,model)        
        all_results.append(status)
    
    all_results = np.array(all_results)     
    final_status = np.sum(all_results,axis=0)
    final_status=final_status/len(models)
    final_status=final_status >= 0.5
    
    write_status(qa_path + image_name + '.csv',final_status,image16.shape[0])
    montage = get_montages(image16,classified_cells,100)
    large_montage = make_output_image(montage)
    io.imsave(qa_path + image_name + "montage.tif",large_montage)
    write_data(image_name,data_writer,final_status,image16.shape[0])
           

def process_directory(image_path,output_path,yolo,config,qa_path,first_live_dead,ld_model_path):
    files = os.listdir(image_path)
    #files = files[26:]
    data_writer,csvfile = prepare_data(qa_path + "Data.csv")
    ld_models = os.listdir(ld_model_path)    
    models = []
    for current_model in ld_models:
        if current_model[-3:] == '.h5':
            print(ld_model_path + '/' + current_model)
            model = load_live_dead_model(ld_model_path + '/' + current_model)
            models.append(model)
    for f in tqdm(files):
        ...
        if not os.path.exists(qa_path + image_name + '.csv'):
            find_cells2(image_path + f, yolo,qa_path,config,output_path,image_name,first_live_dead,models,data_writer)
    csvfile.close()    
...
def _main_(args):
    ...
    if image_path[-4:] == '.mp4':
        ...
        output_path = "/content/gdrive/My Drive/unclassified2/"
        qa_path = "/content/gdrive/My Drive/qa2/"
        image_path = "/content/gdrive/My Drive/Publication/images2/"
        ...
        image_path = "/content/AlignStacks/"
        ld_model_path = "/content/LDModel/"
        firstpass_model_path = "/content/LDModel/DenseNet169.h5"
        image_path   = args.input
        #boxes = yolo.predict(image)
        ...
        first_live_dead = load_live_dead_model(firstpass_model_path)
        process_directory(image_path,output_path,yolo,config,qa_path,first_live_dead,ld_model_path)
