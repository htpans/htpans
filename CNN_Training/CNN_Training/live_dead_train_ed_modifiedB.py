...
from keras.layers import AveragePooling2D, merge, Reshape, Lambda, GlobalAveragePooling2D
...
def VGG_16():
    model_save_path = "LDModelNew/Custom_VGG_16.h5"
...
def create_VGG19():
    model_save_path = "LDModelNew/LDVGG19.h5"
    image_shape=(1,100,100)
    graph_name = 'LDVGG19'
...
def create_InceptionV2():
    model_save_path = "LDModelNew/LDInceptionV2.h5"
...
def create_InceptionV3():
    model_save_path = "LDModelNew/LDInceptionV3.h5"
...
def create_VGG16():
    model_save_path = "LDModelNew/LDVGG16.h5"
    image_shape = (1,100,100)
    graph_name = 'LDVGG16'
...
def create_ResNet50():
    model_save_path = "LDModelNew/LDResNet50.h5"
    image_shape = (1,200,200)
...
def create_Xception():
    ...
    model_save_path = "LDModelNew/Xception.h5"
...
def create_DenseNet121():    
    ...
    model_save_path = "LDModelNew/DenseNet121.h5"
...
def create_DenseNet169():
    ...
    model_save_path = "LDModelNew/DenseNet169.h5"
    image_shape = (1,250,250)
...
def create_DenseNet201():
     #The dense net models use a lot of resources, the batch size needs to be lowered to 32
    model_save_path = "LDModelNew/DenseNet201.h5"
    image_shape = (1,250,250)
...
def train_all():
    model_dir = "/content/gdrive/My Drive/LD_model_Built/"    
    ...
    for f in files:
        ...
        prefix = f[-3:]
        if prefix == '.h5':
            model_save_path = "/content/gdrive/My Drive/LDmodel/" + f
            #image_shape = (200,200,1)
            #graph_name = 'NASNetLarge'
            #model = NASNetLarge(include_top=True, weights=None, input_tensor = Input(shape=image_shape), input_shape = image_shape, classes=2)
            ...
            early_stop = EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=50, verbose=0, mode='auto')
...
def main(unused_argv):
      ...
      if FLAGS.build_model:
          print("Building")
          model,model_save_path,prefix,input_size, channel_order = create_DenseNet201()
          model.save('LDModel_Full/' + prefix +'.h5')
          exit()
      ...
      log_dir = "logs/"      
      model,model_save_path,prefix,input_size, channel_order = create_DenseNet201() #Need to change this line to change the model
      parallel_model = ModelMGPU(model , 2)
      ...
      if True: #while learning_rate > 0.0000001:      
          adam = optimizers.Adam(lr=learning_rate, decay=0.01)      
          parallel_model.compile(loss='sparse_categorical_crossentropy',optimizer = adam , metrics=['accuracy'])     
          early_stop = EarlyStopping(monitor='val_acc', min_delta=0.001, patience=5, verbose=0, mode='auto')
      ...
      #Image.fromarray(image[0][0][0]*255).show()
      #exit()
